  ...
  Lambda: function(lambda) {
    function renderer(self) {
      return function(text) {
        return Hogan.compile(text).render(self, templates);
      };
    }
    return function() {
      return function(text, ctx) {
        try {
          let globalCtx = ctx[0];
          return lambda.call(this, text, renderer(this), globalCtx);
        } catch (error) {
          console.error(error);
        }
      };
  ...
  function createSpecializedPartial(
    ...
    for (key in subs) {
      if (!stackSubs[key]) {
        ...
        stackText[key] =
          parentTpl.activeSub && stackText[parentTpl.activeSub]
            ? stackText[parentTpl.activeSub]
            : parentTpl.text;
      }
    }
...
(function(Hogan) {
  ...
  function tripleStache(node, context) {
    context.code +=
      "t.b(t.t(t." + chooseMethod(node.n) + '("' + esc(node.n) + '",c,p,0)));';
  }
